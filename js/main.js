const countryCodes = {
    "Afghanistan": { code: "+93", length: 9 },
    "Albania": { code: "+355", length: 8 },
    "Algeria": { code: "+213", length: 9 },
    "Andorra": { code: "+376", length: 6 },
    "Angola": { code: "+244", length: 9 },
    "Antigua and Barbuda": { code: "+1-268", length: 7 },
    "Argentina": { code: "+54", length: 10 },
    "Armenia": { code: "+374", length: 8 },
    "Australia": { code: "+61", length: 9 },
    "Austria": { code: "+43", length: 10 },
    "Azerbaijan": { code: "+994", length: 9 },
    "Bahamas": { code: "+1-242", length: 7 },
    "Bahrain": { code: "+973", length: 8 },
    "Bangladesh": { code: "+880", length: 10 },
    "Barbados": { code: "+1-246", length: 7 },
    "Belarus": { code: "+375", length: 9 },
    "Belgium": { code: "+32", length: 9 },
    "Belize": { code: "+501", length: 7 },
    "Benin": { code: "+229", length: 8 },
    "Bhutan": { code: "+975", length: 8 },
    "Bolivia": { code: "+591", length: 8 },
    "Bosnia and Herzegovina": { code: "+387", length: 8 },
    "Botswana": { code: "+267", length: 7 },
    "Brazil": { code: "+55", length: 11 },
    "Brunei": { code: "+673", length: 7 },
    "Bulgaria": { code: "+359", length: 8 },
    "Burkina Faso": { code: "+226", length: 8 },
    "Burundi": { code: "+257", length: 8 },
    "Cabo Verde": { code: "+238", length: 7 },
    "Cambodia": { code: "+855", length: 8 },
    "Cameroon": { code: "+237", length: 9 },
    "Canada": { code: "+1", length: 10 },
    "Central African Republic": { code: "+236", length: 9 },
    "Chad": { code: "+235", length: 8 },
    "Chile": { code: "+56", length: 9 },
    "China": { code: "+86", length: 11 },
    "Colombia": { code: "+57", length: 10 },
    "Comoros": { code: "+269", length: 7 },
    "Congo, Democratic Republic of the": { code: "+243", length: 9 },
    "Congo, Republic of the": { code: "+242", length: 9 },
    "Costa Rica": { code: "+506", length: 8 },
    "Croatia": { code: "+385", length: 9 },
    "Cuba": { code: "+53", length: 8 },
    "Cyprus": { code: "+357", length: 8 },
    "Czech Republic": { code: "+420", length: 9 },
    "Denmark": { code: "+45", length: 8 },
    "Djibouti": { code: "+253", length: 6 },
    "Dominica": { code: "+1-767", length: 7 },
    "Dominican Republic": { code: "+1-809", length: 10 },
    "Ecuador": { code: "+593", length: 9 },
    "Egypt": { code: "+20", length: 10 },
    "El Salvador": { code: "+503", length: 8 },
    "Equatorial Guinea": { code: "+240", length: 9 },
    "Eritrea": { code: "+291", length: 7 },
    "Estonia": { code: "+372", length: 7 },
    "Eswatini": { code: "+268", length: 8 },
    "Ethiopia": { code: "+251", length: 10 },
    "Fiji": { code: "+679", length: 7 },
    "Finland": { code: "+358", length: 9 },
    "France": { code: "+33", length: 9 },
    "Gabon": { code: "+241", length: 7 },
    "Gambia": { code: "+220", length: 7 },
    "Georgia": { code: "+995", length: 9 },
    "Germany": { code: "+49", length: 11 },
    "Ghana": { code: "+233", length: 9 },
    "Greece": { code: "+30", length: 10 },
    "Grenada": { code: "+1-473", length: 7 },
    "Guatemala": { code: "+502", length: 8 },
    "Guinea": { code: "+224", length: 9 },
    "Guinea-Bissau": { code: "+245", length: 7 },
    "Guyana": { code: "+592", length: 7 },
    "Haiti": { code: "+509", length: 8 },
    "Honduras": { code: "+504", length: 8 },
    "Hungary": { code: "+36", length: 9 },
    "Iceland": { code: "+354", length: 7 },
    "India": { code: "+91", length: 10 },
    "Indonesia": { code: "+62", length: 10 },
    "Iran": { code: "+98", length: 10 },
    "Iraq": { code: "+964", length: 10 },
    "Ireland": { code: "+353", length: 9 },
    "Israel": { code: "+972", length: 9 },
    "Italy": { code: "+39", length: 10 },
    "Jamaica": { code: "+1-876", length: 7 },
    "Japan": { code: "+81", length: 11 },
    "Jordan": { code: "+962", length: 9 },
    "Kazakhstan": { code: "+7", length: 10 },
    "Kenya": { code: "+254", length: 10 },
    "Kiribati": { code: "+686", length: 7 },
    "Korea, North": { code: "+850", length: 8 },
    "Korea, South": { code: "+82", length: 11 },
    "Kuwait": { code: "+965", length: 8 },
    "Kyrgyzstan": { code: "+996", length: 9 },
    "Laos": { code: "+856", length: 8 },
    "Latvia": { code: "+371", length: 8 },
    "Lebanon": { code: "+961", length: 8 },
    "Lesotho": { code: "+266", length: 8 },
    "Liberia": { code: "+231", length: 7 },
    "Libya": { code: "+218", length: 9 },
    "Liechtenstein": { code: "+423", length: 9 },
    "Lithuania": { code: "+370", length: 8 },
    "Luxembourg": { code: "+352", length: 8 },
    "Madagascar": { code: "+261", length: 9 },
    "Malawi": { code: "+265", length: 8 },
    "Malaysia": { code: "+60", length: 10 },
    "Maldives": { code: "+960", length: 7 },
    "Mali": { code: "+223", length: 8 },
    "Malta": { code: "+356", length: 8 },
    "Marshall Islands": { code: "+692", length: 7 },
    "Mauritania": { code: "+222", length: 8 },
    "Mauritius": { code: "+230", length: 8 },
    "Mexico": { code: "+52", length: 10 },
    "Micronesia": { code: "+691", length: 7 },
    "Moldova": { code: "+373", length: 8 },
    "Monaco": { code: "+377", length: 7 },
    "Mongolia": { code: "+976", length: 8 },
    "Montenegro": { code: "+382", length: 8 },
    "Morocco": { code: "+212", length: 10 },
    "Mozambique": { code: "+258", length: 9 },
    "Myanmar": { code: "+95", length: 9 },
    "Namibia": { code: "+264", length: 7 },
    "Nauru": { code: "+674", length: 7 },
    "Nepal": { code: "+977", length: 10 },
    "Netherlands": { code: "+31", length: 9 },
    "New Zealand": { code: "+64", length: 9 },
    "Nicaragua": { code: "+505", length: 8 },
    "Niger": { code: "+227", length: 8 },
    "Nigeria": { code: "+234", length: 10 },
    "North Macedonia": { code: "+389", length: 8 },
    "Norway": { code: "+47", length: 8 },
    "Oman": { code: "+968", length: 8 },
    "Pakistan": { code: "+92", length: 10 },
    "Palau": { code: "+680", length: 7 },
    "Panama": { code: "+507", length: 7 },
    "Papua New Guinea": { code: "+675", length: 8 },
    "Paraguay": { code: "+595", length: 9 },
    "Peru": { code: "+51", length: 9 },
    "Philippines": { code: "+63", length: 10 },
    "Poland": { code: "+48", length: 9 },
    "Portugal": { code: "+351", length: 9 },
    "Qatar": { code: "+974", length: 8 },
    "Romania": { code: "+40", length: 10 },
    "Russia": { code: "+7", length: 10 },
    "Rwanda": { code: "+250", length: 9 },
    "Saint Kitts and Nevis": { code: "+1-869", length: 7 },
    "Saint Lucia": { code: "+1-758", length: 7 },
    "Saint Vincent and the Grenadines": { code: "+1-784", length: 7 },
    "Samoa": { code: "+685", length: 7 },
    "San Marino": { code: "+378", length: 7 },
    "Sao Tome and Principe": { code: "+239", length: 7 },
    "Saudi Arabia": { code: "+966", length: 9 },
    "Senegal": { code: "+221", length: 9 },
    "Serbia": { code: "+381", length: 9 },
    "Seychelles": { code: "+248", length: 7 },
    "Sierra Leone": { code: "+232", length: 8 },
    "Singapore": { code: "+65", length: 8 },
    "Slovakia": { code: "+421", length: 9 },
    "Slovenia": { code: "+386", length: 8 },
    "Solomon Islands": { code: "+677", length: 7 },
    "Somalia": { code: "+252", length: 9 },
    "South Africa": { code: "+27", length: 9 },
    "South Sudan": { code: "+211", length: 9 },
    "Spain": { code: "+34", length: 9 },
    "Sri Lanka": { code: "+94", length: 10 },
    "Sudan": { code: "+249", length: 9 },
    "Suriname": { code: "+597", length: 7 },
    "Sweden": { code: "+46", length: 10 },
    "Switzerland": { code: "+41", length: 9 },
    "Syria": { code: "+963", length: 9 },
    "Taiwan": { code: "+886", length: 10 },
    "Tajikistan": { code: "+992", length: 9 },
    "Tanzania": { code: "+255", length: 10 },
    "Thailand": { code: "+66", length: 9 },
    "Togo": { code: "+228", length: 8 },
    "Tonga": { code: "+676", length: 7 },
    "Trinidad and Tobago": { code: "+1-868", length: 7 },
    "Tunisia": { code: "+216", length: 8 },
    "Turkey": { code: "+90", length: 10 },
    "Turkmenistan": { code: "+993", length: 8 },
    "Tuvalu": { code: "+688", length: 7 },
    "Uganda": { code: "+256", length: 9 },
    "Ukraine": { code: "+380", length: 9 },
    "United Arab Emirates": { code: "+971", length: 9 },
    "United Kingdom": { code: "+44", length: 10 },
    "United States": { code: "+1", length: 10 },
    "Uruguay": { code: "+598", length: 8 },
    "Uzbekistan": { code: "+998", length: 9 },
    "Vanuatu": { code: "+678", length: 7 },
    "Vatican City": { code: "+39", length: 7 },
    "Venezuela": { code: "+58", length: 10 },
    "Vietnam": { code: "+84", length: 10 },
    "Yemen": { code: "+967", length: 9 },
    "Zambia": { code: "+260", length: 10 },
    "Zimbabwe": { code: "+263", length: 9 }
};

// Example usage:
const countrySelect = document.getElementById("countryCode");
const phoneInput = document.getElementById("phone");

countrySelect.addEventListener("change", function() {
    const selectedCountry = countryCodes[this.options[this.selectedIndex].text];
    if (selectedCountry) {
        phoneInput.setAttribute("maxlength", selectedCountry.length);
    }
});

// On form submission or on input change, validate the phone number
function validatePhoneNumber() {
    const selectedCountry = countryCodes[countrySelect.options[countrySelect.selectedIndex].text];
    const phoneValue = phoneInput.value.replace(/[^0-9]/g, ""); // Remove non-numeric characters
    if (phoneValue.length !== selectedCountry.length) {
        alert(`Please enter a valid phone number with ${selectedCountry.length} digits for ${selectedCountry.code}`);
        return false;
    }
    return true;
}

// Show the button when the user scrolls down 100px
// Add this script if you want a cool animation effect
$(document).ready(function() {
  $('.whatsapp-float').hover(function() {
    $(this).animate({bottom: '+=5'}, 200);
  }, function() {
    $(this).animate({bottom: '-=5'}, 200);
  });
});